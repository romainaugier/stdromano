...
public:
    ...
    STDROMANO_FORCE_INLINE void stop() noexcept
    {
        if(!stopped)
        {
            log_debug("Scoped profile \"{}\" -> {} {}", this->_name, std::chrono::duration_cast<std::chrono::duration<float, Unit>>(std::chrono::steady_clock::now() - this->_start).count(), TypeName<Unit>().get_name());
            this->stopped = true;
        }
    }
};
...
public:
    ...
    STDROMANO_FORCE_INLINE void stop() noexcept
    {
        if(!stopped)
        {
            log_debug("Scoped profile \"{}\" -> {} {}", this->_name, (uint64_t)(cpu_rdtsc() - this->_start), TypeName<ProfileUnit::Cycles>().get_name());
            this->stopped = true;
        }
    }
};
...
static auto __chrono_func_timer(const char* func_name, F&& func, Args&&... args) noexcept
{
    ...
    log_debug("Func profile \"{}\" -> {} {}", func_name, std::chrono::duration_cast<std::chrono::duration<float, Unit>>(std::chrono::steady_clock::now() - start).count(), TypeName<Unit>().get_name());
...
static auto __cycles_func_timer(const char* func_name, F&& func, Args&&... args) noexcept
{
    ...
    log_debug("Func profile \"{}\" -> {} {}", func_name, (uint64_t)(cpu_rdtsc() - start), TypeName<ProfileUnit::Cycles>().get_name());
...
{
    return __func_timer<Unit>(std::integral_constant<bool, std::is_same<Unit, ProfileUnit::Cycles>::value>{}, func_name, std::forward<F>(func), std::forward<Args>(args)...);
}
...
#define ___LERP_MEAN(X, Y, t) (static_cast<double>(X) + (static_cast<double>(Y) - static_cast<double>(X)) * static_cast<double>(t))
...
#define MEAN_PROFILE_STOP(name) const double ___time_##name = stdromano::get_elapsed_time(___mean_start_##name, 1e9); \
...
#define MEAN_PROFILE_RELEASE(name) stdromano::log_debug("Mean profile \"{}\" -> {:.4} ns", ___mean_##name, ___mean_accum_##name);
#else
