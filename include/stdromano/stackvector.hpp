...
private:
    ...
    void transition_to_heap(size_t new_capacity)
    {
        ...
        this->_heap_storage = mem_aligned_alloc<T>(new_capacity * sizeof(T), alignof(T));
        ...
        {
            ::new(this->_heap_storage + i) T(std::move(reinterpret_cast<T*>(this->_stack_storage)[i]));
    ...
    void grow_if_needed()
    {
        if(this->_size >= this->_capacity)
        {
            ...
            {
                T* new_storage = mem_aligned_alloc<T>(new_capacity * sizeof(T), alignof(T));
...
public:
    ...
    StackVector(StackVector&& other) noexcept : _heap_storage(nullptr)
    {
