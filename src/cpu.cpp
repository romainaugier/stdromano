...
uint32_t _get_cpu_frequency() noexcept
{
    if(g_get_frequency_counter % g_refresh_rate == 0)
    {
        ...
        g_frequency = (std::uint32_t)current;
#elif defined(STDROMANO_LINUX)
...
void cpu_check() noexcept
{
    /* CPU Frequency */
    std::int32_t regs[4];
    std::memset(regs, 0, 4 * sizeof(std::int32_t));
    ...
    if(std::strncmp(cpu_vendor, "GenuineIntel", 12) == 0)
    {
        for(std::int32_t cache_index = 0; cache_index < 10; ++cache_index)
        {
            ...
            std::int32_t cache_type = regs[0] & 0x1F;

            if(cache_type == 0)
                break;

            std::int32_t level = (regs[0] >> 5) & 0x7;
            std::int32_t is_data_or_unified = (cache_type == 1 || cache_type == 3);

            if(!is_data_or_unified)
                continue;

            std::int32_t ways = ((regs[1] >> 22) & 0x3FF) + 1;
            std::int32_t partitions = ((regs[1] >> 12) & 0x3FF) + 1;
            std::int32_t line_size = (regs[1] & 0xFFF) + 1;
            std::int32_t sets = regs[2] + 1;
    ...
    {
        std::int32_t max_ext_leaf;
        cpuid(regs, 0x80000000);
        max_ext_leaf = regs[0];

        if (max_ext_leaf >= static_cast<std::int32_t>(0x80000005))
        {
            cpuid(regs, 0x80000005);
            g_cpu_caches_sizes[0] = (regs[2] >> 24) * 1024;
        }

        if (max_ext_leaf >= static_cast<std::int32_t>(0x80000006))
        {
...
bool cpu_get_name(char* name) noexcept
{
    ...
    if(regs[0] < static_cast<std::int32_t>(0x80000004))
    {
        name[0] = '\0';
        return false;
    }
    ...
    std::memcpy(name, regs, 12 * sizeof(std::uint32_t));

    name[12 * sizeof(std::uint32_t)] = '\0';
